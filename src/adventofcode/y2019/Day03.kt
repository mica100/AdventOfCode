...
class Pointer(var x: Int, var y: Int) {
    ...
    operator fun get(i: Int): Int {
        return when (i) {
            0 -> x
            1 -> y
            else -> throw IndexOutOfBoundsException("index can be 0 or 1")
        }
    }
    ...
    override fun equals(other: Any?): Boolean {
        return Pair<Int,Int>(x,y).equals(other)
    }

    override fun hashCode(): Int {
        return Pair<Int,Int>(x,y).hashCode()
    }
}

class Field(
        private var pointerChar: Char = LABEL_A,
        private val arr: HashMap<Pair<Int,Int>, Char> = HashMap(),
        private var pointer: Pointer = Pointer(0, 0),
        private var crossList: MutableList<Pair<Int, Int>> = mutableListOf() ) {

    private fun getCoord(x: Int): Int { return x }
    fun getLabel(x: Int): Int { return x }

    fun set(x: Int, y: Int, c: Char? = null): Field {
        if (c != null) pointerChar = c

        //println("set(${getLabel(x)}, ${getLabel(y)}")
        var char = pointerChar
        val x1 = getCoord(x)
        val y1 = getCoord(y)
        val pointer = Pair(x1, y1)
        val oldChar = arr[pointer]

        if (oldChar == LABEL_CROSS) char = LABEL_CROSS
        if (char == LABEL_A && oldChar == LABEL_B || char == LABEL_B && oldChar == LABEL_A) {
            char = LABEL_CROSS
            crossList.add(pointer)
        }
    ...
    fun setPointer(char: Char): Field {
        pointerChar = char
        return this
    }
    fun setPointer(x: Int, y: Int, char: Char? = null): Field {
        pointer.x = x
        pointer.y = y
        if (char != null) {
            set(x, y, char)
    ...
    infix fun up(len: Int): Field {
        movePointer("U$len")
        return this
    }
    infix fun down(len: Int): Field {
        movePointer("D$len")
        return this
    }
    infix fun right(len: Int): Field {
        movePointer("R$len")
        return this
    }
    infix fun left(len: Int): Field {
        movePointer("L$len")
        return this
    }
    infix fun move(code: String): Field {
        movePointer(code)
        return this
    }
    fun movePointer(code: String) {
        val direction = code[0]
        val len = code.substring(1).toInt()

        println("${pointer.x},${pointer.y}: ${code}")
        val pointerNew = pointer.clone()
        when(direction.toUpperCase()) {
            'R' -> { pointerNew.x += len; for(_x in pointer.x until  pointerNew.x) set(_x, pointer.y) }
            'L' -> { pointerNew.x -= len; for(_x in pointer.x downTo pointerNew.x) set(_x, pointer.y) }
            'D' -> { pointerNew.y -= len; for(_y in pointer.y downTo pointerNew.y) set(pointer.x, _y) }
            'U' -> { pointerNew.y += len; for(_y in pointer.y until  pointerNew.y) set(pointer.x, _y) }
            else -> throw UnsupportedOperationException("Move operation $direction unknown!")
        }
        pointer = pointerNew
    }

    fun process(command: String) {
        for (code in command.split(",")) {
            movePointer(code)
        }
    }

    fun findNearest(): Pair<Pair<Int,Int>, Int>? {
        val base = Pair<Int,Int>(1,1)
        class Entry(val p: Pointer, val dist: Int)
        var minDist: Int = Int.MAX_VALUE
        var minPointer: Pair<Int,Int>? = null

        for (elem in crossList) {
            val distX = abs(base.first - elem.first)
            val distY = abs(base.second - elem.second)
            val dist = distX + distY
            if (dist < minDist) {
                minPointer = elem
                minDist = dist
            }
        }
        if (minPointer != null) return Pair(minPointer, minDist)
        return null
    }

    fun crossPointCount(): Int {
